...
async function loadRawData(config, bq) {
  ...
  Logger.info(`Loading raw data into table: ${tableName}`);
...
async function generateLoadSummary(config, bq) {
  ...
  for (const category of categories) {
    ...
    if (categoryTables.length > 0) {
      ...
      for (const table of categoryTables) {
        ...
        Logger.success(`  ✅ ${table.name}: ${formattedCount} rows - ${table.description}`);
      }
    }
  }
  ...
  Logger.success(`  • Total main data rows: ${totalRows.toLocaleString()}`);
  Logger.success(`  • Tables loaded: ${loadedTables.length}`);
  Logger.success(`  • Lookup tables: ${loadedTables.filter(t => t.type === 'Lookup').length}`);
  Logger.success(`  • SDR mapping tables: ${loadedTables.filter(t => t.type.includes('SDR')).length}`);

  // Data source information
  const sourceUri = config.gcs.transformDest ?
    config.gcs.transformDest + config.gcs.sourceUri.split('/').pop() :
    config.gcs.sourceUri;
  Logger.info(`  • Data source: ${sourceUri}${config.gcs.transformDest ? ' (preprocessed)' : ' (raw)'}`);

  // Warning for missing tables
  if (missingTables.length > 0) {
    console.log();
    Logger.warn(`⚠️  Missing or failed tables (${missingTables.length}):`);
    for (const table of missingTables) {
      const errorMsg = table.error ? ` (${table.error})` : '';
      Logger.warn(`  • ${table.name}${errorMsg}`);
    }
  }
